...
const UserTable: React.FC<UserTableProps> = ({ topicId }) => {
    ...
    debounce(async (user: string, topic: string) => {
      try {
        const response = await fetch(
          `http://localhost:3000/user-topic/delete`,
          {
            method: "PATCH",
            headers: {
        ...
        if (response.ok) {
          toast.success("Remove User From Topic successfully.");
          fetchData(topic); // Gọi lại fetchData sau khi xóa thành công
        } else {
          const errorResponse = await response.json(); // In lỗi từ API
          console.error("Failed to Remove User From Topic:", errorResponse);
  ...
  const fetchData = async (topic: string) => {
    try {
      const response = await fetch(
        `http://localhost:3000/user-topic/getusers?topic_id=${topic}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const users = data.metadata.map((item: any) => item.user);
      setData(users);
    } catch (error) {
