...
import { getImagesBySectionId } from "@/src/util/actions/imageAction";
...
const ImageComponent: React.FC<ImageProps> = ({ id }) => {
  ...
  const debouncedFetchImages = debounce(async (sectionId: string) => {
    try {
      const response = await getImagesBySectionId(sectionId);
